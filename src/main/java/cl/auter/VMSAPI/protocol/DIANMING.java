...
System.out.println("sendFile: " + commandHead + " " + fileName + " " + contents.toString());		
		do {
			...
			if (bytesRead > 0) {
				...
				do {
					this.clean();
					this.command = new DIANMINGPackage(this.destinationAddress, this.sourceAddress, commandHead,
							ArrayUtils.toObject(byteSend));
					ok = sendPackage();
					if (!ok) {
						attempt++;
						Thread.sleep(1500);
...
//            VMSDAO dao = new VMSDAO();
	...
	public String toString() {
		return "DIANMING{" + "urlServidor=" + serverURL + ", puerto=" + portURL + ", socket=" + socket
				+ ", direccionObjetivo=" + destinationAddress + ", direccionFuente=" + sourceAddress + ", modoConexion="
				+ connectionMode + '}';
	}
